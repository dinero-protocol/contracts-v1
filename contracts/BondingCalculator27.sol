...
contract BondingCalculator27 is IBondingCalculator {
    ...
    function valuation( address _pair, uint amount_ ) external view override returns ( uint _value ) {
        ...
        _value = totalValue.mul( FixedPoint.fraction( amount_, totalSupply ).decode112with18() ).div( 1e27 );
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == BTRFLY ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( BTRFLY ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}
