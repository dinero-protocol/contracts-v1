...
contract LockToken is ILockToken, ERC721, Auth{
    ...
    function setLockStatus(uint lockId, bool enable) external override requiresAuth {
        LockInfo storage lockToUpdate = _lockInfos[lockId];
        lockToUpdate.enabled = enable;
    }

    function createNewLock(LockInfo memory newLock) external override requiresAuth {
        _lockInfos[lockTypeCount++] = newLock;
    }
    ...
    function _mintLock(uint lockId, uint amount, address to) internal returns(uint nftId){
    ...
    function _mergeLock(uint[] calldata nftIds) internal returns(uint nftId){
        ...
        for(uint i=1; i < nftIds.length; i++){
            ...
            if (nftInfo.expiry > latestExpiry) latestExpiry = nftInfo.expiry;
