...
describe("Live CVX bonds", function(){
    ...
    it(`MinPrice of ${MINPRICE} gives [Zeus] a discount between 15% & 25% out the gate`, async function(){
        ...
        const cvxDepositBtrflyValue = 
        (ethers.utils.parseUnits('100','gwei')).mul(cvxValueUSD).div(btrflyValueUSD)

        const redemptionMinValue = cvxDepositBtrflyValue.
        mul(BigNumber.from(117)).div(BigNumber.from(100))

        const redemptionMaxValue = cvxDepositBtrflyValue.
        mul(BigNumber.from(133)).div(BigNumber.from(100))
        ...
        await cvxBond.connect(cvxWhale).deposit(
